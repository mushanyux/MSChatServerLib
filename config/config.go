...
type Config struct {
	...
	MuShanIM struct {
		APIURL       string // im基地址
		ManagerToken string // im的管理者token mushanim配置了就需要填写，没配置就不需要
	}
	// ---------- 头像 ----------
	Avatar struct {
		Default        string // 默认头像
...
func (c *Config) ConfigureWithViper(vp *viper.Viper) {
	...
	//#################### IM ####################
	c.MuShanIM.APIURL = c.getString("mushanIM.apiURL", c.MuShanIM.APIURL)
	c.MuShanIM.ManagerToken = c.getString("mushanIM.managerToken", c.MuShanIM.ManagerToken)
