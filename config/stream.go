...
func (c *Context) IMStreamStart(req MessageStreamStartReq) (string, error) {
	resp, err := network.Post(c.cfg.MuShanIM.APIURL+"/streammessage/start", []byte(util.ToJson(req)), nil)
	if err != nil {
		return "", err
	}
...
func (c *Context) IMStreamEnd(req MessageStreamEndReq) error {
	resp, err := network.Post(c.cfg.MuShanIM.APIURL+"/streammessage/end", []byte(util.ToJson(req)), nil)
	if err != nil {
		return err
	}
	err = c.handlerIMError(resp)
